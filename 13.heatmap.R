...
sample_read_sums <- read.table("11.normalize.read_sums.xls", header=T, sep="\t", row.names=1)
sample_read_sums
dim(sample_read_sums)

best.tax <- read.table("12.best_tax.xls", header=T, sep='\t', row.names=1)
best.tax$desc <- sprintf("%s : %s (%s, %.2g)", rownames(best.tax), best.tax$best_tax, best.tax$best_tax_level, best.tax$best_tax_confidence)

best.tax$desc

raw_otus <- read.table("syncom_itags.xls", header=T, sep='\t', row.names=1)
...
log_otus <- read.table("syncom_itags.log_normalized.xls", header=T, sep='\t', row.names=1)
...
# must be a t least 2% in any sample
keepers <- as.vector(apply(otus_pcnt[, -1], MARGIN = 1, function(x) any(x > .02)))
otus_pcnt <- otus_pcnt[keepers, ]
log_otus <- log_otus[keepers, ]
